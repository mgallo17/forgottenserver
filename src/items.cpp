...
FILELOADER_ERRORS Items::loadFromOtb(const std::string& file)
{
	...
	if (majorVersion == 0xFFFFFFFF) {
		...
		std::cout << "[Warning - Items::loadFromOtb] Old major version, loading anyway." << std::endl;
		// allow old version
	} else if (minorVersion < CLIENT_VERSION_1098) {
		std::cout << "[Warning - Items::loadFromOtb] Old minor version, loading anyway." << std::endl;
		// allow old version
	}
